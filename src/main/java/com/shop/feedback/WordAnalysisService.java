...
import com.shop.repository.IWordAnalysisService;
...
public class WordAnalysisService implements IWordAnalysisService {
    ...
    private Logger log = Logger.getLogger("WordAnalysisService");
    //자연어 처리 - 형태소 분석기인 Komoran를 메모리에 올리기 위해 WordAnalysisService 클래스 내 전역 변수로 설정합니다.
    Komoran nlp = null;
    // 톰켓에서 부팅할 때 @Service를 모두 메모리에 올립니다.
    ...
    public WordAnalysisService() {
        ...
        this.nlp = new Komoran(DEFAULT_MODEL.LIGHT);    // 학습데이터 경량화 버전( 웹 서비스에 적합. )
    ...
    public List<String> doWordNouns(String text) throws Exception {
        ...
        while (it.hasNext()) {
            //추출된 명서
            String word = Feedback.nvl(it.next());
